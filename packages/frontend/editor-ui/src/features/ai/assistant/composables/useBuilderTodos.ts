...
export function useBuilderTodos() {
	...
	const baseWorkflowIssues = computed(() => {
		...
		return workflowDocumentStore.value.workflowValidationIssues.filter(
			(issue) =>
	...
	const hasTodosHiddenByPinnedData = computed(() => {
		...
		const wouldHaveBaseIssues = workflowDocumentStore.value.workflowValidationIssues.some(
			(issue) =>
				['credentials', 'parameters'].includes(issue.type) &&
				nodeHasPinnedData(issue.node) &&
				!nodeIsDisabled(issue.node),
	...
	function getTodosToTrack(): TodosTrackingPayload {
		const credentials_todo_count = workflowDocumentStore.value.workflowValidationIssues.filter(
