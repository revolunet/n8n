...
describe('workflowDocument.store orchestration', () => {
	...
	describe('workflowValidationIssues', () => {
		it('collects issues only from connected, enabled nodes', () => {
			...
			const issues = workflowDocumentStore.workflowValidationIssues;
	...
	describe('formatIssueMessage', () => {
		it('joins array entries and trims trailing period', () => {
			...
			const message = workflowDocumentStore.formatIssueMessage(['Missing URL', 'Invalid value.']);
		...
		it('returns string representation for non-array values', () => {
			...
			expect(workflowDocumentStore.formatIssueMessage('Simple issue.')).toBe('Simple issue.');
		});
	});
	...
	describe('nodesIssuesExist', () => {
		it('should return true when a node has issues and connected', () => {
			...
			const hasIssues = workflowDocumentStore.nodesIssuesExist;
